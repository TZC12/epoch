...
//
// 官方原样落盘，只动两处，均标 【本地改动】：
//   F1 动作可以没有图标（我们的"删除"是文字动作，且 label 已 i18n）；
//   F2 dragEnabled —— pager 长按武装期本行让位（见 ui/swipe.ts 的仲裁说明）。
// 其余常量、阈值、DOM 结构一律照抄：改一处就要重新对齐一次官方版本，不值当。
...
export type SwipeAction = {
  id: string;
  label: ReactNode;
  /** 【本地改动】官方必填：没有图标就呈现 label（见 SwipeActionButton）。 */
  icon?: ReactNode;
...
export interface SwipeableListProps {
  ...
  /**
   * 【本地改动】手势仲裁开关。官方假设这条列表独占横滑；Epoch 的行还嵌在
   * SegmentedPager 里，pager 长按 400ms 武装后要翻卡，行必须立刻停手。
   *
   * 只需要翻转 drag 这个布尔，不需要 cancel：motion 每帧 move 都重读
   * getProps().drag，shouldDrag() 见 drag=false 直接 return，指针继续走也只是
   * 停在原地；松手仍走正常 onDragEnd → 结算回原位。中途关掉不会留下半拖状态。
   */
  dragEnabled?: boolean;
  className?: string;
...
function SwipeActionButton({
  ...
  const textOnly = action.icon === undefined; /* 【本地改动】F1 */
  return (
    <button
      type="button"
    ...
    >
      <span
        className={cn(
          "grid place-items-center transition-[background-color,color,transform] duration-150 group-hover:bg-background group-active:scale-95",
          /* 【本地改动】F1：图标动作保持官方 36px 圆；文字动作铺满按钮，
             整块露出面都可点（旧版删除 pill 就是这个尺寸关系）。 */
          !textOnly && "h-9 w-9 rounded-full",
          ACTION_TONE_CLASS[action.tone ?? "neutral"],
        )}
      >
        {action.icon ?? action.label}
      </span>
      {/* 文字动作时 label 已经可见，再来一份 sr-only 就是双读。 */}
      {textOnly ? null : <span className="sr-only">{action.label}</span>}
    </button>
  );
}
...
function SwipeableListRow({
  ...
  return (
    ...
    >
      ...
      <motion.div
        drag={item.disabled || !dragEnabled ? false : "x"}
